...
/*   Updated: 2023/05/29 19:58:49 by abrisse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void last_vertical_intercept(t_ray *ray, t_data *data)
{
	...
	{
		if (has_wall_at(data, ray->intercept.x, ray->intercept.y - ray->is_facing_left))
		{
...
static void last_horiztonal_intercept(t_ray *ray, t_data *data)
{
	while (ray->intercept.x >= 0 && ray->intercept.x <= data->graphic.win_width
		&& ray->intercept.y >= 0 && ray->intercept.y <= data->graphic.win_height)
	{
...
void	horizontal_intercept(t_ray *ray, t_data *data)
{
	...
	if (!ray->is_facing_left && ray->step.x < 0)
		ray->step.x *= -1;
	last_horiztonal_intercept(ray, data);
}
